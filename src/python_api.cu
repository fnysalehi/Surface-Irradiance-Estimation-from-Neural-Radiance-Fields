...
PYBIND11_MODULE(pyngp, m) {
	...
	py::class_<Sdf> sdf(testbed, "Sdf");
	sdf
		.def_readonly("training", &Sdf::training)
		.def_readwrite("mesh_sdf_mode", &Sdf::mesh_sdf_mode)
		.def_readwrite("mesh_scale", &Sdf::mesh_scale)
		.def_readwrite("analytic_normals", &Sdf::analytic_normals)
		.def_readwrite("shadow_sharpness", &Sdf::shadow_sharpness)
		.def_readwrite("fd_normals_epsilon", &Sdf::fd_normals_epsilon)
		.def_readwrite("use_triangle_octree", &Sdf::use_triangle_octree)
		.def_readwrite("zero_offset", &Sdf::zero_offset)
		.def_readwrite("distance_scale", &Sdf::distance_scale)
		.def_readwrite("calculate_iou_online", &Sdf::calculate_iou_online)
		.def_readwrite("groundtruth_mode", &Sdf::groundtruth_mode)
		.def_readwrite("brick_level", &Sdf::brick_level)
		.def_readonly("brick_res", &Sdf::brick_res)
		.def_readwrite("brdf", &Sdf::brdf)
		;

	py::class_<Sdf::Training>(sdf, "Training")
		.def_readwrite("generate_sdf_data_online", &Sdf::Training::generate_sdf_data_online)
		.def_readwrite("surface_offset_scale", &Sdf::Training::surface_offset_scale)
		;
