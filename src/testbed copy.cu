...
void Testbed::train(uint32_t batch_size) {
	...
	if (m_training_step % n_prep_to_skip == 0) {
		...
		switch (m_testbed_mode) {
			...
			case ETestbedMode::Geometry: training_prep_nerf_geometry(batch_size, m_stream.get()); break;
...

	{
		...
		switch (m_testbed_mode) {
			...
			case ETestbedMode::Geometry: train_sdf_geometry(batch_size, get_loss_scalar, m_stream.get()); 
	...
	switch (m_testbed_mode) {
		...
		case ETestbedMode::Volume:
			...
			case ETestbedMode::Geometry:
			{
				...
				// );
				if (!m_render_ground_truth || m_ground_truth_alpha < 1.0f) {
					render_geometry_nerf(device.stream(), device, device.render_buffer_view(), device.geometry_nerf_network(), device.data().density_grid_bitfield_ptr, focal_length, camera_matrix0, camera_matrix1, nerf_rolling_shutter, screen_center, foveation, visualized_dimension);
				}
