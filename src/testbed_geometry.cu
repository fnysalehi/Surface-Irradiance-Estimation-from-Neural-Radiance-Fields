...
void Testbed::render_geometry_mesh(
	...
	// BoundingBox mesh_bounding_box = m_aabb; 
	...
	mesh_bounding_box.inflate(m_geometry.sdf.zero_offset);
	...
	if (render_mode == ERenderMode::Shade || render_mode == ERenderMode::Normals) {
		...
		if (render_mode == ERenderMode::Shade && n_hit > 0) {
			...
			shadow_tracer.set_shadow_sharpness(m_geometry.sdf.shadow_sharpness);
...
void Testbed::load_mesh(MeshData* mesh, const fs::path& data_path, vec3 center) {
	...
	*///////////////////////////////////////////////////////////////
	...
	m_geometry.sdf.triangles_cpu.resize(n_triangles);
	for (size_t i = 0; i < n_vertices; i += 3) {
		m_geometry.sdf.triangles_cpu[i/3] = {vertices[i+0], vertices[i+1], vertices[i+2]};
	}

	if (!m_geometry.sdf.triangle_bvh) {
		m_geometry.sdf.triangle_bvh = TriangleBvh::make();
	}

	m_geometry.sdf.triangle_bvh->build(m_geometry.sdf.triangles_cpu, 8);
	m_geometry.sdf.triangles_gpu.resize_and_copy_from_host(m_geometry.sdf.triangles_cpu);
	m_geometry.sdf.triangle_bvh->build_optix(m_geometry.sdf.triangles_gpu, m_stream.get());

	m_geometry.sdf.triangle_octree.reset(new TriangleOctree{});
	m_geometry.sdf.triangle_octree->build(*m_geometry.sdf.triangle_bvh, m_geometry.sdf.triangles_cpu, 10);
	m_geometry.sdf.brick_data.free_memory();

	m_bounding_radius = length(vec3(0.5f));

	// Compute discrete probability distribution for later sampling of the mesh's surface
	m_geometry.sdf.triangle_weights.resize(n_triangles);
	for (size_t i = 0; i < n_triangles; ++i) {
		m_geometry.sdf.triangle_weights[i] = m_geometry.sdf.triangles_cpu[i].surface_area();
	}
	m_geometry.sdf.triangle_distribution.build(m_geometry.sdf.triangle_weights);

	// Move CDF to gpu
	m_geometry.sdf.triangle_cdf.resize_and_copy_from_host(m_geometry.sdf.triangle_distribution.cdf);

	// Clear training data as it's no longer representative
	// of the previously loaded mesh... but don't clear the network.
	// Perhaps it'll look interesting while morphing from one mesh to another.
	m_geometry.sdf.training.idx = 0;
	m_geometry.sdf.training.size = 0;
...
void Testbed::generate_training_samples_sdf_geometry(vec3* positions, float* distances, uint32_t n_to_generate, cudaStream_t stream, bool uniform_only) {
	...
	linear_kernel(sample_uniform_on_triangle_kernel_geometry, 0, stream,
		...
		m_geometry.sdf.triangle_cdf.data(),
		(uint32_t)m_geometry.sdf.triangle_cdf.size(),
		m_geometry.sdf.triangles_gpu.data(),
		positions
	);
	...
	float stddev = m_bounding_radius/1024.0f * m_geometry.sdf.training.surface_offset_scale;
	if (!uniform_only && (m_geometry.sdf.uses_takikawa_encoding || m_geometry.sdf.use_triangle_octree)) {
		linear_kernel(uniform_octree_sample_kernel_geometry, 0, stream,
			n_to_generate_uniform,
			m_rng,
			m_geometry.sdf.triangle_octree->nodes_gpu(),
			m_geometry.sdf.triangle_octree->n_nodes(),
			m_geometry.sdf.triangle_octree->depth(),
		...
		float leaf_size = scalbnf(1.0f, -m_geometry.sdf.triangle_octree->depth()+1);
		if (leaf_size < stddev) {
		...
		sdf_aabb.inflate(m_geometry.sdf.zero_offset);
	...
	m_geometry.sdf.training.perturbations.enlarge(n_to_generate_surface_offset);
	generate_random_logistic<float>(stream, m_rng, n_to_generate_surface_offset*3, (float*)m_geometry.sdf.training.perturbations.data(), 0.0f, stddev);

	linear_kernel(perturb_mesh_samples, 0, stream,
		n_to_generate_surface_offset,
		m_geometry.sdf.training.perturbations.data(),
	...
	m_geometry.sdf.triangle_bvh->signed_distance_gpu(
		n_to_generate_uniform+n_to_generate_surface_offset,
		m_geometry.sdf.mesh_sdf_mode,
		positions+n_to_generate_surface_exact,
		distances+n_to_generate_surface_exact,
		m_geometry.sdf.triangles_gpu.data(),
		true,
...
GPUMemory<float> Testbed::get_sdf_gt_on_grid_geometry(ivec3 res3d, const BoundingBox& aabb, const mat3& render_aabb_to_local) {
	...
	mesh.triangle_bvh->signed_distance_gpu(
			n_elements,
			m_geometry.sdf.mesh_sdf_mode,
			positions,
			sdf_out,
...
void Testbed::train_sdf_geometry(size_t target_batch_size, bool get_loss_scalar, cudaStream_t stream) {
	...
	if (m_geometry.sdf.training.size >= target_batch_size) {
		// Auxiliary matrices for training
		const uint32_t batch_size = (uint32_t)std::min(m_geometry.sdf.training.size, target_batch_size);

		// Permute all training records to de-correlate training data
		linear_kernel(shuffle<vec3>, 0, stream, m_geometry.sdf.training.size, 1, m_training_step, m_geometry.sdf.training.positions.data(), m_geometry.sdf.training.positions_shuffled.data());
		linear_kernel(shuffle<float>, 0, stream, m_geometry.sdf.training.size, 1, m_training_step, m_geometry.sdf.training.distances.data(), m_geometry.sdf.training.distances_shuffled.data());

		GPUMatrix<float> training_target_matrix(m_geometry.sdf.training.distances_shuffled.data(), n_output_dims, batch_size);
		GPUMatrix<float> training_batch_matrix((float*)(m_geometry.sdf.training.positions_shuffled.data()), n_input_dims, batch_size);
...
void Testbed::training_prep_geometry(uint32_t batch_size, cudaStream_t stream) {
	if (m_geometry.sdf.training.generate_sdf_data_online) {
		m_geometry.sdf.training.size = batch_size;
		m_geometry.sdf.training.positions.enlarge(m_geometry.sdf.training.size);
		m_geometry.sdf.training.positions_shuffled.enlarge(m_geometry.sdf.training.size);
		m_geometry.sdf.training.distances.enlarge(m_geometry.sdf.training.size);
		m_geometry.sdf.training.distances_shuffled.enlarge(m_geometry.sdf.training.size);

		generate_training_samples_sdf_geometry(m_geometry.sdf.training.positions.data(), m_geometry.sdf.training.distances.data(), batch_size, stream, false);
	}
}
...
// 	m_geometry.sdf.iou_counter.enlarge(8);
// 	if (!blocking) // when not blocking, returns data from the *last* run, then kicks off work to accumulate some more samples
// 		cudaMemcpy(countercpu, m_geometry.sdf.iou_counter.data(), 8*4, cudaMemcpyDeviceToHost);

// 	if (scale_existing_results_factor < 1.f) {
// 		linear_kernel(scale_iou_counters_kernel, 0, stream, 8, m_geometry.sdf.iou_counter.data(), scale_existing_results_factor);
// 	}
// 	while (n_samples>0) {
...
// 		m_geometry.sdf.training.positions.enlarge(m_geometry.sdf.training.size);
// 		m_geometry.sdf.training.distances.enlarge(m_geometry.sdf.training.size); // we use this buffer for the GT distances
// 		m_geometry.sdf.training.distances_shuffled.enlarge(m_geometry.sdf.training.size); // we use the shuffled output for the output of inference

// 		generate_training_samples_sdf(m_geometry.sdf.training.positions.data(), m_geometry.sdf.training.distances.data(), (uint32_t)(batch_size), stream, true);
// 		GPUMatrix<float> positions_matrix((float*)m_geometry.sdf.training.positions.data(), 3, batch_size);
// 		GPUMatrix<float> distances_matrix(m_geometry.sdf.training.distances_shuffled.data(), 1, batch_size);
// 		m_network->inference(stream, positions_matrix, distances_matrix);
// 		auto* octree_ptr = (m_geometry.sdf.uses_takikawa_encoding || m_geometry.sdf.use_triangle_octree || force_use_octree) ? m_geometry.sdf.triangle_octree.get() : nullptr;
// 		linear_kernel(compare_signs_kernel,0, stream, batch_size, m_geometry.sdf.training.positions.data(),
// 			m_geometry.sdf.training.distances.data(), //  ref
// 			m_geometry.sdf.training.distances_shuffled.data(), // model
// 			m_geometry.sdf.iou_counter.data(),
// 			octree_ptr ? octree_ptr->nodes_gpu() : nullptr, octree_ptr ? octree_ptr->depth() : 0);
// 	}
// 	if (blocking) {
// 		CUDA_CHECK_THROW(cudaMemcpyAsync(countercpu, m_geometry.sdf.iou_counter.data(), 8*4, cudaMemcpyDeviceToHost, stream));
// 		CUDA_CHECK_THROW(cudaStreamSynchronize(stream));
// 	}
