...
uint32_t Testbed::MyTracer::trace_bvh(TriangleBvh* bvh, const Triangle* triangles, cudaStream_t stream) {
	...
	bvh->ray_trace_gpu(n_alive, m_rays_mesh[0].pos, m_rays_mesh[0].normal, triangles, stream);
	return n_alive;
}
...
void Testbed::render_geometry_mesh(
	...
	// tlog::info() << "bounding_box.min: (" << bounding_box.min.x << ", " << bounding_box.min.y << ", " << bounding_box.min.z << ")";
	// tlog::info() << "bounding_box.max: (" << bounding_box.max.x << ", " << bounding_box.max.y << ", " << bounding_box.max.z << ")";
...
void Testbed::MyTracer::init_rays_from_camera_nerf(
	...
	init_rays_with_payload_kernel_nerf_geometry<<<blocks, threads, 0, stream>>>(
		...
		parallax_shift,
		snap_to_pixel_centers,
		render_aabb,			//maybe this and the next one should change!
...
void Testbed::render_geometry_nerf(
	...
	if (render_2d) {
		...
		linear_kernel(generate_nerf_network_inputs_at_current_position_geometry, 0, stream, n_hit, m_geometry.nerfBoundingBox, rays_hit.payload, PitchedPtr<NerfCoordinate>((NerfCoordinate*)positions_matrix.data(), 1, 0, extra_stride), extra_dims_gpu);

		if (visualized_dimension == -1) {
			nerf_network->inference(stream, positions_matrix, rgbsigma_matrix);
			linear_kernel(compute_nerf_rgba_kernel_geometry, 0, stream, n_hit, (vec4*)rgbsigma_matrix.data(), m_geometry.nerf.rgb_activation, m_geometry.nerf.density_activation, 0.01f, false);
		} else {
...
void Testbed::load_nerf_post(Nerf* nerf, const vec3 center) { // moved the second half of load_nerf here
	...
	m_geometry.nerfBoundingBox.inflate(0.5f * std::min(1 << (NERF_CASCADES()-1), (*nerf).training.dataset.aabb_scale));
	// m_raw_aabb = m_aabb;
...
void Testbed::load_scene(const fs::path& data_path) {
	...
	*/
	...
	// 	m_render_aabb = BoundingBox{vec3(0.5f), vec3(0.5f)};
	// 	m_render_aabb.inflate(0.5f);
	// 	m_render_aabb_to_local = mat3::identity();
...
void Testbed::generate_training_samples_sdf_geometry(vec3* positions, float* distances, uint32_t n_to_generate, cudaStream_t stream, bool uniform_only) {
	...
	float stddev = m_bounding_radius/1024.0f * m_geometry.mesh_cpu[0].sdf.training.surface_offset_scale;
	if (!uniform_only && (m_geometry.mesh_cpu[0].sdf.uses_takikawa_encoding || m_geometry.mesh_cpu[0].sdf.use_triangle_octree)) {
		linear_kernel(uniform_octree_sample_kernel_geometry, 0, stream,
			n_to_generate_uniform,
			m_rng,
			m_geometry.mesh_cpu[0].sdf.triangle_octree->nodes_gpu(),
			m_geometry.mesh_cpu[0].sdf.triangle_octree->n_nodes(),
			m_geometry.mesh_cpu[0].sdf.triangle_octree->depth(),
		...
		float leaf_size = scalbnf(1.0f, -m_geometry.mesh_cpu[0].sdf.triangle_octree->depth()+1);
		if (leaf_size < stddev) {
			tlog::warning() << "leaf_size < stddev";
			stddev = leaf_size;
		}

		linear_kernel(assign_float_geometry, 0, stream,
			n_to_generate_uniform,
		...
		sdf_aabb.inflate(m_geometry.mesh_cpu[0].sdf.zero_offset);
		linear_kernel(scale_to_aabb_kernel_geometry, 0, stream,
	...
	m_geometry.mesh_cpu[0].sdf.training.perturbations.enlarge(n_to_generate_surface_offset);
	generate_random_logistic<float>(stream, m_rng, n_to_generate_surface_offset*3, (float*)m_geometry.mesh_cpu[0].sdf.training.perturbations.data(), 0.0f, stddev);

	linear_kernel(perturb_mesh_samples, 0, stream,
		n_to_generate_surface_offset,
		m_geometry.mesh_cpu[0].sdf.training.perturbations.data(),
...
__global__ void generate_grid_samples_sdf_uniform_geometry(ivec3 res_3d, BoundingBox aabb, const mat3& render_aabb_to_local, vec3* __restrict__ out) {
	uint32_t x = threadIdx.x + blockIdx.x * blockDim.x;
	uint32_t y = threadIdx.y + blockIdx.y * blockDim.y;
	uint32_t z = threadIdx.z + blockIdx.z * blockDim.z;
	if (x>=res_3d.x || y>=res_3d.y || z>=res_3d.z)
...
// not important
GPUMemory<float> Testbed::get_sdf_gt_on_grid_geometry(ivec3 res3d, const BoundingBox& aabb, const mat3& render_aabb_to_local) {
	...
	generate_grid_samples_sdf_uniform_geometry<<<blocks, threads, 0, m_stream.get()>>>(res3d, aabb, render_aabb_to_local, positions);
	CUDA_CHECK_THROW(cudaStreamSynchronize(m_stream.get()));
	
	const MeshData& mesh = m_geometry.mesh_cpu[0];

	const TriangleBvhNode* bvhnodes = mesh.sdf.triangle_bvh->nodes_gpu();
	const Triangle* triangles = mesh.sdf.triangles_gpu.data();
	
	mesh.sdf.triangle_bvh->signed_distance_gpu(
			n_elements,
			m_geometry.mesh_cpu[0].sdf.mesh_sdf_mode,
			positions,
			sdf_out,
...
void Testbed::train_sdf_geometry(size_t target_batch_size, bool get_loss_scalar, cudaStream_t stream) {
	const uint32_t n_output_dims = 1;
	const uint32_t n_input_dims = 3;

	if (m_geometry.mesh_cpu[0].sdf.training.size >= target_batch_size) {
		// Auxiliary matrices for training
		const uint32_t batch_size = (uint32_t)std::min(m_geometry.mesh_cpu[0].sdf.training.size, target_batch_size);

		// Permute all training records to de-correlate training data
		linear_kernel(shuffle<vec3>, 0, stream, m_geometry.mesh_cpu[0].sdf.training.size, 1, m_training_step, m_geometry.mesh_cpu[0].sdf.training.positions.data(), m_geometry.mesh_cpu[0].sdf.training.positions_shuffled.data());
		linear_kernel(shuffle<float>, 0, stream, m_geometry.mesh_cpu[0].sdf.training.size, 1, m_training_step, m_geometry.mesh_cpu[0].sdf.training.distances.data(), m_geometry.mesh_cpu[0].sdf.training.distances_shuffled.data());

		GPUMatrix<float> training_target_matrix(m_geometry.mesh_cpu[0].sdf.training.distances_shuffled.data(), n_output_dims, batch_size);
		GPUMatrix<float> training_batch_matrix((float*)(m_geometry.mesh_cpu[0].sdf.training.positions_shuffled.data()), n_input_dims, batch_size);
...
void Testbed::training_prep_geometry(uint32_t batch_size, cudaStream_t stream) {
	if (m_geometry.mesh_cpu[0].sdf.training.generate_sdf_data_online) {
		m_geometry.mesh_cpu[0].sdf.training.size = batch_size;
		m_geometry.mesh_cpu[0].sdf.training.positions.enlarge(m_geometry.mesh_cpu[0].sdf.training.size);
		m_geometry.mesh_cpu[0].sdf.training.positions_shuffled.enlarge(m_geometry.mesh_cpu[0].sdf.training.size);
		m_geometry.mesh_cpu[0].sdf.training.distances.enlarge(m_geometry.mesh_cpu[0].sdf.training.size);
		m_geometry.mesh_cpu[0].sdf.training.distances_shuffled.enlarge(m_geometry.mesh_cpu[0].sdf.training.size);

		generate_training_samples_sdf_geometry(m_geometry.mesh_cpu[0].sdf.training.positions.data(), m_geometry.mesh_cpu[0].sdf.training.distances.data(), batch_size, stream, false);
	}
}
...
// nerf
/////////////////////////////////////////////////////
// void Testbed::update_density_grid_nerf_geometry(float decay, uint32_t n_uniform_density_grid_samples, uint32_t n_nonuniform_density_grid_samples, cudaStream_t stream) {
// 	const uint32_t n_elements = NERF_GRID_N_CELLS() * (m_geometry.nerf.max_cascade + 1);

// 	m_geometry.nerf.density_grid.resize(n_elements);

// 	const uint32_t n_density_grid_samples = n_uniform_density_grid_samples + n_nonuniform_density_grid_samples;

// 	const uint32_t padded_output_width = m_geometry_nerf_network->padded_density_output_width();
...
// 	if (m_training_step == 0 || m_geometry.nerf.training.n_images_for_training != m_geometry.nerf.training.n_images_for_training_prev) {
// 		m_geometry.nerf.training.n_images_for_training_prev = m_geometry.nerf.training.n_images_for_training;
// 		if (m_training_step == 0) {
// 			m_geometry.nerf.density_grid_ema_step = 0;
// 		}
// 		// Only cull away empty regions where no camera is looking when the cameras are actually meaningful.
// 		if (!m_geometry.nerf.training.dataset.has_rays) {
// 			linear_kernel(mark_untrained_density_grid_geometry, 0, stream, n_elements, m_geometry.nerf.density_grid.data(),
// 				m_geometry.nerf.training.n_images_for_training,
// 				m_geometry.nerf.training.dataset.metadata_gpu.data(),
// 				m_geometry.nerf.training.transforms_gpu.data(),
// 				m_training_step == 0
// 			);
// 		} else {
// 			CUDA_CHECK_THROW(cudaMemsetAsync(m_geometry.nerf.density_grid.data(), 0, sizeof(float)*n_elements, stream));
// 		}
// 	}
...
// 		linear_kernel(generate_grid_samples_nerf_nonuniform_geometry, 0, stream,
// 			n_uniform_density_grid_samples,
// 			m_geometry.nerf.training.density_grid_rng,
// 			m_geometry.nerf.density_grid_ema_step,
// 			m_geometry.nerfBoundingBox,
// 			m_geometry.nerf.density_grid.data(),
// 			density_grid_positions,
// 			density_grid_indices,
// 			m_geometry.nerf.max_cascade+1,
// 			-0.01f
// 		);
// 		m_geometry.nerf.training.density_grid_rng.advance();
...
// 			m_geometry_nerf_network->density(stream, density_grid_position_matrix, density_matrix, false);
// 		}

// 		linear_kernel(splat_grid_samples_nerf_max_nearest_neighbor_geometry, 0, stream, n_density_grid_samples, density_grid_indices, mlp_out, density_grid_tmp, m_geometry.nerf.rgb_activation, m_geometry.nerf.density_activation);
// 		linear_kernel(ema_grid_samples_nerf_geometry, 0, stream, n_elements, decay, m_geometry.nerf.density_grid_ema_step, m_geometry.nerf.density_grid.data(), density_grid_tmp);
...
// void Testbed::train_nerf_geometry(uint32_t target_batch_size, bool get_loss_scalar, cudaStream_t stream) {
// 	if (m_geometry.nerf.training.n_images_for_training == 0) {
// 		tlog::warning() << "No images for training. Aborting training.";
// 		return;
// 	}

// 	if (m_geometry.nerf.training.include_sharpness_in_error) {
// 		tlog::info() << "Training sharpness";
// 		size_t n_cells = NERF_GRID_N_CELLS() * NERF_CASCADES();
// 		if (m_geometry.nerf.training.sharpness_grid.size() < n_cells) {
// 			m_geometry.nerf.training.sharpness_grid.enlarge(NERF_GRID_N_CELLS() * NERF_CASCADES());
// 			CUDA_CHECK_THROW(cudaMemsetAsync(m_geometry.nerf.training.sharpness_grid.data(), 0, m_geometry.nerf.training.sharpness_grid.get_bytes(), stream));
// 		}

// 		if (m_training_step == 0) {
// 			CUDA_CHECK_THROW(cudaMemsetAsync(m_geometry.nerf.training.sharpness_grid.data(), 0, m_geometry.nerf.training.sharpness_grid.get_bytes(), stream));
// 		} else {
// 			linear_kernel(decay_sharpness_grid_nerf_geometry, 0, stream, m_geometry.nerf.training.sharpness_grid.size(), 0.95f, m_geometry.nerf.training.sharpness_grid.data());
// 		}
// 	}
// 	tlog::info() << "prepare for training steps in geometry mode";
// 	m_geometry.nerf.training.counters_rgb.prepare_for_training_steps(stream);

// 	if (m_geometry.nerf.training.n_steps_since_cam_update == 0) {
// 		CUDA_CHECK_THROW(cudaMemsetAsync(m_geometry.nerf.training.cam_pos_gradient_gpu.data(), 0, m_geometry.nerf.training.cam_pos_gradient_gpu.get_bytes(), stream));
// 		CUDA_CHECK_THROW(cudaMemsetAsync(m_geometry.nerf.training.cam_rot_gradient_gpu.data(), 0, m_geometry.nerf.training.cam_rot_gradient_gpu.get_bytes(), stream));
// 		CUDA_CHECK_THROW(cudaMemsetAsync(m_geometry.nerf.training.cam_exposure_gradient_gpu.data(), 0, m_geometry.nerf.training.cam_exposure_gradient_gpu.get_bytes(), stream));
...
// 		CUDA_CHECK_THROW(cudaMemsetAsync(m_geometry.nerf.training.cam_focal_length_gradient_gpu.data(), 0, m_geometry.nerf.training.cam_focal_length_gradient_gpu.get_bytes(), stream));
// 	}

// 	bool train_extra_dims = m_geometry.nerf.training.dataset.n_extra_learnable_dims > 0 && m_geometry.nerf.training.optimize_extra_dims;
// 	uint32_t n_extra_dims = m_geometry.nerf.training.dataset.n_extra_dims();
// 	if (train_extra_dims) {
// 		uint32_t n = n_extra_dims * m_geometry.nerf.training.n_images_for_training;
// 		m_geometry.nerf.training.extra_dims_gradient_gpu.enlarge(n);
// 		CUDA_CHECK_THROW(cudaMemsetAsync(m_geometry.nerf.training.extra_dims_gradient_gpu.data(), 0, m_geometry.nerf.training.extra_dims_gradient_gpu.get_bytes(), stream));
// 	}

// 	if (m_geometry.nerf.training.n_steps_since_error_map_update == 0 && !m_geometry.nerf.training.dataset.metadata.empty()) {
// 		uint32_t n_samples_per_image = (m_geometry.nerf.training.n_steps_between_error_map_updates * m_geometry.nerf.training.counters_rgb.rays_per_batch) / m_geometry.nerf.training.dataset.n_images;
// 		ivec2 res = m_geometry.nerf.training.dataset.metadata[0].resolution;
// 		m_geometry.nerf.training.error_map.resolution = min(ivec2((int)(std::sqrt(std::sqrt((float)n_samples_per_image)) * 3.5f)), res);
// 		m_geometry.nerf.training.error_map.data.resize(product(m_geometry.nerf.training.error_map.resolution) * m_geometry.nerf.training.dataset.n_images);
// 		CUDA_CHECK_THROW(cudaMemsetAsync(m_geometry.nerf.training.error_map.data.data(), 0, m_geometry.nerf.training.error_map.data.get_bytes(), stream));
// 	}

// 	float* envmap_gradient = m_geometry.nerf.training.train_envmap ? m_envmap.envmap->gradients() : nullptr;
// 	if (envmap_gradient) {
// 		CUDA_CHECK_THROW(cudaMemsetAsync(envmap_gradient, 0, sizeof(float)*m_envmap.envmap->n_params(), stream));
// 	}
// 	tlog::info() << "Training Nerf in geometry mode";
// 	train_nerf_step_geometry(target_batch_size, m_geometry.nerf.training.counters_rgb, stream);
...
// 	float loss_scalar = m_geometry.nerf.training.counters_rgb.update_after_training(target_batch_size, get_loss_scalar, stream);
// 	bool zero_records = m_geometry.nerf.training.counters_rgb.measured_batch_size == 0;
// 	if (get_loss_scalar) {
...
// 	m_geometry.nerf.training.n_steps_since_error_map_update += 1;
...
// 	if (accumulate_error && m_geometry.nerf.training.n_steps_since_error_map_update >= m_geometry.nerf.training.n_steps_between_error_map_updates) {
// 		m_geometry.nerf.training.error_map.cdf_resolution = m_geometry.nerf.training.error_map.resolution;
// 		m_geometry.nerf.training.error_map.cdf_x_cond_y.resize(product(m_geometry.nerf.training.error_map.cdf_resolution) * m_geometry.nerf.training.dataset.n_images);
// 		m_geometry.nerf.training.error_map.cdf_y.resize(m_geometry.nerf.training.error_map.cdf_resolution.y * m_geometry.nerf.training.dataset.n_images);
// 		m_geometry.nerf.training.error_map.cdf_img.resize(m_geometry.nerf.training.dataset.n_images);

// 		CUDA_CHECK_THROW(cudaMemsetAsync(m_geometry.nerf.training.error_map.cdf_x_cond_y.data(), 0, m_geometry.nerf.training.error_map.cdf_x_cond_y.get_bytes(), stream));
// 		CUDA_CHECK_THROW(cudaMemsetAsync(m_geometry.nerf.training.error_map.cdf_y.data(), 0, m_geometry.nerf.training.error_map.cdf_y.get_bytes(), stream));
// 		CUDA_CHECK_THROW(cudaMemsetAsync(m_geometry.nerf.training.error_map.cdf_img.data(), 0, m_geometry.nerf.training.error_map.cdf_img.get_bytes(), stream));

// 		const dim3 threads = { 16, 8, 1 };
// 		const dim3 blocks = { div_round_up((uint32_t)m_geometry.nerf.training.error_map.cdf_resolution.y, threads.x), div_round_up((uint32_t)m_geometry.nerf.training.dataset.n_images, threads.y), 1 };
// 		construct_cdf_2d_geometry<<<blocks, threads, 0, stream>>>(
// 			m_geometry.nerf.training.dataset.n_images, m_geometry.nerf.training.error_map.cdf_resolution.y, m_geometry.nerf.training.error_map.cdf_resolution.x,
// 			m_geometry.nerf.training.error_map.data.data(),
// 			m_geometry.nerf.training.error_map.cdf_x_cond_y.data(),
// 			m_geometry.nerf.training.error_map.cdf_y.data()
// 		);
// 		linear_kernel(construct_cdf_1d_geometry, 0, stream,
// 			m_geometry.nerf.training.dataset.n_images,
// 			m_geometry.nerf.training.error_map.cdf_resolution.y,
// 			m_geometry.nerf.training.error_map.cdf_y.data(),
// 			m_geometry.nerf.training.error_map.cdf_img.data()
// 		);

// 		// Compute image CDF on the CPU. It's single-threaded anyway. No use parallelizing.
// 		m_geometry.nerf.training.error_map.pmf_img_cpu.resize(m_geometry.nerf.training.error_map.cdf_img.size());
// 		m_geometry.nerf.training.error_map.cdf_img.copy_to_host(m_geometry.nerf.training.error_map.pmf_img_cpu);
// 		std::vector<float> cdf_img_cpu = m_geometry.nerf.training.error_map.pmf_img_cpu; // Copy unnormalized PDF into CDF buffer
// 		float cum = 0;
...
// 			m_geometry.nerf.training.error_map.pmf_img_cpu[i] = (1.0f - MIN_PMF) * m_geometry.nerf.training.error_map.pmf_img_cpu[i] * norm + MIN_PMF / (float)m_geometry.nerf.training.dataset.n_images;
// 			cdf_img_cpu[i] = (1.0f - MIN_PMF) * cdf_img_cpu[i] * norm + MIN_PMF * (float)(i+1) / (float)m_geometry.nerf.training.dataset.n_images;
// 		}
// 		m_geometry.nerf.training.error_map.cdf_img.copy_from_host(cdf_img_cpu);
...
// 		m_geometry.nerf.training.error_map.is_cdf_valid = true;

// 		m_geometry.nerf.training.n_steps_between_error_map_updates = (uint32_t)(m_geometry.nerf.training.n_steps_between_error_map_updates * 1.5f);
// 	}

// 	// Get extrinsics gradients
// 	m_geometry.nerf.training.n_steps_since_cam_update += 1;

// 	if (train_extra_dims) {
// 		std::vector<float> extra_dims_gradient(m_geometry.nerf.training.extra_dims_gradient_gpu.size());
// 		m_geometry.nerf.training.extra_dims_gradient_gpu.copy_to_host(extra_dims_gradient);

// 		// Optimization step
// 		for (uint32_t i = 0; i < m_geometry.nerf.training.n_images_for_training; ++i) {
...
// 			//float l2_reg = 1e-4f;
// 			//gradient += m_geometry.nerf.training.extra_dims_opt[i].variable() * l2_reg;

// 			m_geometry.nerf.training.extra_dims_opt[i].set_learning_rate(m_optimizer->learning_rate());
// 			m_geometry.nerf.training.extra_dims_opt[i].step(gradient);
// 		}

// 		m_geometry.nerf.training.update_extra_dims();
// 	}

// 	bool train_camera = m_geometry.nerf.training.optimize_extrinsics || m_geometry.nerf.training.optimize_distortion || m_geometry.nerf.training.optimize_focal_length || m_geometry.nerf.training.optimize_exposure;
// 	if (train_camera && m_geometry.nerf.training.n_steps_since_cam_update >= m_geometry.nerf.training.n_steps_between_cam_updates) {
// 		float per_camera_loss_scale = (float)m_geometry.nerf.training.n_images_for_training / LOSS_SCALE() / (float)m_geometry.nerf.training.n_steps_between_cam_updates;

// 		if (m_geometry.nerf.training.optimize_extrinsics) {
// 			CUDA_CHECK_THROW(cudaMemcpyAsync(m_geometry.nerf.training.cam_pos_gradient.data(), m_geometry.nerf.training.cam_pos_gradient_gpu.data(), m_geometry.nerf.training.cam_pos_gradient_gpu.get_bytes(), cudaMemcpyDeviceToHost, stream));
// 			CUDA_CHECK_THROW(cudaMemcpyAsync(m_geometry.nerf.training.cam_rot_gradient.data(), m_geometry.nerf.training.cam_rot_gradient_gpu.data(), m_geometry.nerf.training.cam_rot_gradient_gpu.get_bytes(), cudaMemcpyDeviceToHost, stream));

// 			CUDA_CHECK_THROW(cudaStreamSynchronize(stream));

// 			// Optimization step
// 			for (uint32_t i = 0; i < m_geometry.nerf.training.n_images_for_training; ++i) {
// 				vec3 pos_gradient = m_geometry.nerf.training.cam_pos_gradient[i] * per_camera_loss_scale;
// 				vec3 rot_gradient = m_geometry.nerf.training.cam_rot_gradient[i] * per_camera_loss_scale;

// 				float l2_reg = m_geometry.nerf.training.extrinsic_l2_reg;
// 				pos_gradient += m_geometry.nerf.training.cam_pos_offset[i].variable() * l2_reg;
// 				rot_gradient += m_geometry.nerf.training.cam_rot_offset[i].variable() * l2_reg;

// 				m_geometry.nerf.training.cam_pos_offset[i].set_learning_rate(std::max(m_geometry.nerf.training.extrinsic_learning_rate * std::pow(0.33f, (float)(m_geometry.nerf.training.cam_pos_offset[i].step() / 128)), m_optimizer->learning_rate()/1000.0f));
// 				m_geometry.nerf.training.cam_rot_offset[i].set_learning_rate(std::max(m_geometry.nerf.training.extrinsic_learning_rate * std::pow(0.33f, (float)(m_geometry.nerf.training.cam_rot_offset[i].step() / 128)), m_optimizer->learning_rate()/1000.0f));

// 				m_geometry.nerf.training.cam_pos_offset[i].step(pos_gradient);
// 				m_geometry.nerf.training.cam_rot_offset[i].step(rot_gradient);
// 			}

// 			m_geometry.nerf.training.update_transforms();
// 		}

// 		if (m_geometry.nerf.training.optimize_distortion) {
// 			linear_kernel(safe_divide_geometry, 0, stream,
// 				m_distortion.map->n_params(),
// 				m_distortion.map->gradients(),
// 				m_distortion.map->gradient_weights()
// 			);
// 			m_distortion.trainer->optimizer_step(stream, LOSS_SCALE() * (float)m_geometry.nerf.training.n_steps_between_cam_updates);
// 		}

// 		if (m_geometry.nerf.training.optimize_focal_length) {
// 			CUDA_CHECK_THROW(cudaMemcpyAsync(&m_geometry.nerf.training.cam_focal_length_gradient, m_geometry.nerf.training.cam_focal_length_gradient_gpu.data(), m_geometry.nerf.training.cam_focal_length_gradient_gpu.get_bytes(), cudaMemcpyDeviceToHost, stream));
// 			CUDA_CHECK_THROW(cudaStreamSynchronize(stream));
// 			vec2 focal_length_gradient = m_geometry.nerf.training.cam_focal_length_gradient * per_camera_loss_scale;
// 			float l2_reg = m_geometry.nerf.training.intrinsic_l2_reg;
// 			focal_length_gradient += m_geometry.nerf.training.cam_focal_length_offset.variable() * l2_reg;
// 			m_geometry.nerf.training.cam_focal_length_offset.set_learning_rate(std::max(1e-3f * std::pow(0.33f, (float)(m_geometry.nerf.training.cam_focal_length_offset.step() / 128)),m_optimizer->learning_rate() / 1000.0f));
// 			m_geometry.nerf.training.cam_focal_length_offset.step(focal_length_gradient);
// 			m_geometry.nerf.training.dataset.update_metadata();
// 		}

// 		if (m_geometry.nerf.training.optimize_exposure) {
// 			CUDA_CHECK_THROW(cudaMemcpyAsync(m_geometry.nerf.training.cam_exposure_gradient.data(), m_geometry.nerf.training.cam_exposure_gradient_gpu.data(), m_geometry.nerf.training.cam_exposure_gradient_gpu.get_bytes(), cudaMemcpyDeviceToHost, stream));

// 			vec3 mean_exposure = vec3(0.0f);

// 			// Optimization step
// 			for (uint32_t i = 0; i < m_geometry.nerf.training.n_images_for_training; ++i) {
// 				vec3 gradient = m_geometry.nerf.training.cam_exposure_gradient[i] * per_camera_loss_scale;

// 				float l2_reg = m_geometry.nerf.training.exposure_l2_reg;
// 				gradient += m_geometry.nerf.training.cam_exposure[i].variable() * l2_reg;

// 				m_geometry.nerf.training.cam_exposure[i].set_learning_rate(m_optimizer->learning_rate());
// 				m_geometry.nerf.training.cam_exposure[i].step(gradient);

// 				mean_exposure += m_geometry.nerf.training.cam_exposure[i].variable();
// 			}

// 			mean_exposure /= (float)m_geometry.nerf.training.n_images_for_training;

// 			// Renormalize
// 			std::vector<vec3> cam_exposures(m_geometry.nerf.training.n_images_for_training);
// 			for (uint32_t i = 0; i < m_geometry.nerf.training.n_images_for_training; ++i) {
// 				cam_exposures[i] = m_geometry.nerf.training.cam_exposure[i].variable() -= mean_exposure;
// 			}

// 			CUDA_CHECK_THROW(cudaMemcpyAsync(m_geometry.nerf.training.cam_exposure_gpu.data(), cam_exposures.data(), m_geometry.nerf.training.n_images_for_training * sizeof(vec3), cudaMemcpyHostToDevice, stream));
// 		}
...
// void Testbed::train_nerf_step_geometry(uint32_t target_batch_size, NerfCounters& counters, cudaStream_t stream) {
// 	tlog::info() << "Training Nerf step in geometry mode ";
// 	const uint32_t padded_output_width = m_geometry_nerf_network->padded_output_width();
...
// 	m_geometry.nerf.training.n_rays_since_error_map_update += counters.rays_per_batch;

// 	// If we have an envmap, prepare its gradient buffer
// 	float* envmap_gradient = m_geometry.nerf.training.train_envmap ? m_envmap.envmap->gradients() : nullptr;

// 	bool sample_focal_plane_proportional_to_error = m_geometry.nerf.training.error_map.is_cdf_valid && m_geometry.nerf.training.sample_focal_plane_proportional_to_error;
// 	bool sample_image_proportional_to_error = m_geometry.nerf.training.error_map.is_cdf_valid && m_geometry.nerf.training.sample_image_proportional_to_error;
// 	bool include_sharpness_in_error = m_geometry.nerf.training.include_sharpness_in_error;
// 	// This is low-overhead enough to warrant always being on.
// 	// It makes for useful visualizations of the training error.
// 	bool accumulate_error = true;
...
// 	{
// 		linear_kernel(generate_training_samples_nerf_geometry, 0, stream,
// 			counters.rays_per_batch,
// 			m_geometry.nerfBoundingBox,
// 			max_inference,
// 			n_rays_total,
...
// 			m_geometry.nerf.training.dataset.metadata_gpu.data(),
// 			m_geometry.nerf.training.transforms_gpu.data(),
// 			m_geometry.nerf.density_grid_bitfield.data(),
// 			m_geometry.nerf.max_cascade,
// 			m_max_level_rand_training,
// 			max_level,
// 			m_geometry.nerf.training.snap_to_pixel_centers,
// 			m_geometry.nerf.training.train_envmap,
// 			m_geometry.nerf.cone_angle_constant,
// 			m_distortion.view(),
// 			sample_focal_plane_proportional_to_error ? m_geometry.nerf.training.error_map.cdf_x_cond_y.data() : nullptr,
// 			sample_focal_plane_proportional_to_error ? m_geometry.nerf.training.error_map.cdf_y.data() : nullptr,
// 			sample_image_proportional_to_error ? m_geometry.nerf.training.error_map.cdf_img.data() : nullptr,
// 			m_geometry.nerf.training.error_map.cdf_resolution,
// 			m_geometry.nerf.training.extra_dims_gpu.data(),
...
// 		linear_kernel(compute_loss_kernel_train_nerf_geometry, 0, stream,
// 			counters.rays_per_batch,
// 			m_geometry.nerfBoundingBox,
// 			n_rays_total,
// 			m_rng,
...
// 			m_color_space,
// 			m_geometry.nerf.training.random_bg_color,
// 			m_geometry.nerf.training.linear_colors,
// 			m_geometry.nerf.training.n_images_for_training,
// 			m_geometry.nerf.training.dataset.metadata_gpu.data(),
// 			mlp_out,
...
// 			dloss_dmlp_out,
// 			m_geometry.nerf.training.loss_type,
// 			m_geometry.nerf.training.depth_loss_type,
// 			counters.loss.data(),
// 			m_max_level_rand_training,
// 			max_level_compacted,
// 			m_geometry.nerf.rgb_activation,
// 			m_geometry.nerf.density_activation,
// 			m_geometry.nerf.training.snap_to_pixel_centers,
// 			accumulate_error ? m_geometry.nerf.training.error_map.data.data() : nullptr,
// 			sample_focal_plane_proportional_to_error ? m_geometry.nerf.training.error_map.cdf_x_cond_y.data() : nullptr,
// 			sample_focal_plane_proportional_to_error ? m_geometry.nerf.training.error_map.cdf_y.data() : nullptr,
// 			sample_image_proportional_to_error ? m_geometry.nerf.training.error_map.cdf_img.data() : nullptr,
// 			m_geometry.nerf.training.error_map.resolution,
// 			m_geometry.nerf.training.error_map.cdf_resolution,
// 			include_sharpness_in_error ? m_geometry.nerf.training.dataset.sharpness_data.data() : nullptr,
// 			m_geometry.nerf.training.dataset.sharpness_resolution,
// 			m_geometry.nerf.training.sharpness_grid.data(),
// 			m_geometry.nerf.density_grid.data(),
// 			m_geometry.nerf.density_grid_mean.data(),
// 			m_geometry.nerf.max_cascade,
// 			m_geometry.nerf.training.cam_exposure_gpu.data(),
// 			m_geometry.nerf.training.optimize_exposure ? m_geometry.nerf.training.cam_exposure_gradient_gpu.data() : nullptr,
// 			m_geometry.nerf.training.depth_supervision_lambda,
// 			m_geometry.nerf.training.near_distance
// 		);
// 	}
...
// 	bool train_camera = m_geometry.nerf.training.optimize_extrinsics || m_geometry.nerf.training.optimize_distortion || m_geometry.nerf.training.optimize_focal_length;
// 	bool train_extra_dims = m_geometry.nerf.training.dataset.n_extra_learnable_dims > 0 && m_geometry.nerf.training.optimize_extra_dims;
...
// 		linear_kernel(compute_extra_dims_gradient_train_nerf_geometry, 0, stream,
// 			counters.rays_per_batch,
// 			n_rays_total,
// 			ray_counter,
// 			m_geometry.nerf.training.extra_dims_gradient_gpu.data(),
// 			m_geometry.nerf.training.dataset.n_extra_dims(),
// 			m_geometry.nerf.training.n_images_for_training,
// 			ray_indices,
// 			numsteps,
...
// 		linear_kernel(compute_cam_gradient_train_nerf_geometry, 0, stream,
// 			counters.rays_per_batch,
// 			n_rays_total,
// 			m_rng,
// 			m_geometry.nerfBoundingBox,
...
// 			m_geometry.nerf.training.transforms_gpu.data(),
// 			m_geometry.nerf.training.snap_to_pixel_centers,
// 			m_geometry.nerf.training.optimize_extrinsics ? m_geometry.nerf.training.cam_pos_gradient_gpu.data() : nullptr,
// 			m_geometry.nerf.training.optimize_extrinsics ? m_geometry.nerf.training.cam_rot_gradient_gpu.data() : nullptr,
// 			m_geometry.nerf.training.n_images_for_training,
// 			m_geometry.nerf.training.dataset.metadata_gpu.data(),
// 			ray_indices,
// 			rays_unnormalized,
...
// 			PitchedPtr<NerfCoordinate>((NerfCoordinate*)coords_compacted, 1, 0, extra_stride),
// 			PitchedPtr<NerfCoordinate>((NerfCoordinate*)coords_gradient, 1, 0, extra_stride),
// 			m_geometry.nerf.training.optimize_distortion ? m_distortion.map->gradients() : nullptr,
// 			m_geometry.nerf.training.optimize_distortion ? m_distortion.map->gradient_weights() : nullptr,
// 			m_distortion.resolution,
// 			m_geometry.nerf.training.optimize_focal_length ? m_geometry.nerf.training.cam_focal_length_gradient_gpu.data() : nullptr,
// 			sample_focal_plane_proportional_to_error ? m_geometry.nerf.training.error_map.cdf_x_cond_y.data() : nullptr,
// 			sample_focal_plane_proportional_to_error ? m_geometry.nerf.training.error_map.cdf_y.data() : nullptr,
// 			sample_image_proportional_to_error ? m_geometry.nerf.training.error_map.cdf_img.data() : nullptr,
// 			m_geometry.nerf.training.error_map.cdf_resolution
...
// void Testbed::training_prep_nerf_geometry(uint32_t batch_size, cudaStream_t stream) {
// 	if (m_geometry.nerf.training.n_images_for_training == 0) {
// 		return;
// 	}

// 	float alpha = m_geometry.nerf.training.density_grid_decay;
// 	uint32_t n_cascades = m_geometry.nerf.max_cascade+1;

// 	if (m_training_step < 256) {
// 		update_density_grid_nerf_geometry(alpha, NERF_GRID_N_CELLS() * n_cascades, 0, stream);
// 	} else {
// 		update_density_grid_nerf_geometry(alpha, NERF_GRID_N_CELLS() / 4 * n_cascades, NERF_GRID_N_CELLS() / 4 * n_cascades, stream);
// 	}
// }
