...
void Testbed::render_geometry_mesh(
	...
	bool gt_raytrace = m_render_ground_truth && m_geometry.groundtruth_mode == ESDFGroundTruthMode::RaytracedMesh;
	
	auto trace = [&](MyTracer& tracer) {
		if (gt_raytrace) {
			// return tracer.trace_bvh(m_geometry.mesh_cpu[0].sdf.triangle_bvh.get(), m_geometry.mesh_cpu[0].sdf.triangles_gpu.data(), stream);
...
// 	m_geometry.mesh_cpu[0].sdf.iou_counter.enlarge(8);
// 	if (!blocking) // when not blocking, returns data from the *last* run, then kicks off work to accumulate some more samples
// 		cudaMemcpy(countercpu, m_geometry.mesh_cpu[0].sdf.iou_counter.data(), 8*4, cudaMemcpyDeviceToHost);

// 	if (scale_existing_results_factor < 1.f) {
// 		linear_kernel(scale_iou_counters_kernel, 0, stream, 8, m_geometry.mesh_cpu[0].sdf.iou_counter.data(), scale_existing_results_factor);
// 	}
// 	while (n_samples>0) {
...
// 		m_geometry.mesh_cpu[0].sdf.training.positions.enlarge(m_geometry.mesh_cpu[0].sdf.training.size);
// 		m_geometry.mesh_cpu[0].sdf.training.distances.enlarge(m_geometry.mesh_cpu[0].sdf.training.size); // we use this buffer for the GT distances
// 		m_geometry.mesh_cpu[0].sdf.training.distances_shuffled.enlarge(m_geometry.mesh_cpu[0].sdf.training.size); // we use the shuffled output for the output of inference

// 		generate_training_samples_sdf(m_geometry.mesh_cpu[0].sdf.training.positions.data(), m_geometry.mesh_cpu[0].sdf.training.distances.data(), (uint32_t)(batch_size), stream, true);
// 		GPUMatrix<float> positions_matrix((float*)m_geometry.mesh_cpu[0].sdf.training.positions.data(), 3, batch_size);
// 		GPUMatrix<float> distances_matrix(m_geometry.mesh_cpu[0].sdf.training.distances_shuffled.data(), 1, batch_size);
// 		m_network->inference(stream, positions_matrix, distances_matrix);
// 		auto* octree_ptr = (m_geometry.mesh_cpu[0].sdf.uses_takikawa_encoding || m_geometry.mesh_cpu[0].sdf.use_triangle_octree || force_use_octree) ? m_geometry.mesh_cpu[0].sdf.triangle_octree.get() : nullptr;
// 		linear_kernel(compare_signs_kernel,0, stream, batch_size, m_geometry.mesh_cpu[0].sdf.training.positions.data(),
// 			m_geometry.mesh_cpu[0].sdf.training.distances.data(), //  ref
// 			m_geometry.mesh_cpu[0].sdf.training.distances_shuffled.data(), // model
// 			m_geometry.mesh_cpu[0].sdf.iou_counter.data(),
// 			octree_ptr ? octree_ptr->nodes_gpu() : nullptr, octree_ptr ? octree_ptr->depth() : 0);
// 	}
// 	if (blocking) {
// 		CUDA_CHECK_THROW(cudaMemcpyAsync(countercpu, m_geometry.mesh_cpu[0].sdf.iou_counter.data(), 8*4, cudaMemcpyDeviceToHost, stream));
// 		CUDA_CHECK_THROW(cudaStreamSynchronize(stream));
// 	}
